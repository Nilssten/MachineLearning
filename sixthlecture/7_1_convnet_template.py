...
class Conv2d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        out_height = get_out_size(in_height, self.padding, self.kernel_size, self.stride)
        out_width = get_out_size(in_width, self.padding, self.kernel_size, self.stride)
            ...
            for k in range(self.out_channels):
                for i in range(out_height):
                    for j in range(out_width):
                        ...
                        out[b, k, i, j] = (x_padded[b, :, h_start:h_end, w_start:w_end] * self.K[k]).sum()
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.encoder = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            Conv2d(in_channels=5, out_channels=5, kernel_size=4, stride=2, padding=1),
            torch.nn.ReLU(),
            Conv2d(in_channels=5, out_channels=10, kernel_size=4, stride=2, padding=1),
            torch.nn.ReLU()
        )

        # Calculation of the flattened features size
        in_features_fc = get_out_size(in_size=28, kernel_size=3, stride=1, padding=1)  # First conv layer
        in_features_fc = get_out_size(in_size=in_features_fc, kernel_size=4, stride=2, padding=1)  # Second conv layer
        in_features_fc = get_out_size(in_size=in_features_fc, kernel_size=4, stride=2, padding=1)  # Third conv layer

        # Calculate the total flattened size for the Linear layer
        self.fc = torch.nn.Linear(
            in_features=in_features_fc * in_features_fc * 10,  # Adjust the calculation if the dimensions differ
            out_features=67  # Number of output classes or features
        )

    def forward(self, x):
        batch_size = x.size(0)
        features = self.encoder(x)
        flatten = features.view(batch_size, -1)
